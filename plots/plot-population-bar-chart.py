...
POPULATION_COLUMNS = [
    ("experimental_population_at_final_timestep_mean", "experimental_population_at_final_timestep_stdDev", "experimental", "magenta"),
    ("control_population_at_final_timestep_mean", "control_population_at_final_timestep_stdDev", "control", "cyan"),
]
TOTAL_POPULATION_COLUMN = ("total_population_at_final_timestep_mean", "total_population_at_final_timestep_stdDev", "total", "cyan")
...
def print_help():
    print(
        "Usage:\n\tpython plot-averaged-population-csv.py --path /path/to/population.csv [--output /path/to/output.pdf]\n\n"
        "Options:\n"
        "\t-p,--path\tUse the specified CSV file produced by print-reporting.py's population output.\n"
        ...
        "\t-y,--ylabel\tOptional custom y-axis label. Defaults to Mean Population at Final Timestep.\n"
        ...
        "\t-t,--total-only\tPlot only total popuation as a single line.\n"
...
def load_population_csv(csv_path, total_only=False):
    with open(csv_path, newline="") as input_file:
        ...
        configurations = []
        columns = [TOTAL_POPULATION_COLUMN] if total_only is True else POPULATION_COLUMNS
...
def default_output_path(input_path):
    ...
    return os.path.join(os.path.dirname(input_path), f"{base_name}_averaged_population.pdf")
...
def generate_bar_chart(
    ...
    configuration_column, configurations, series = load_population_csv(csv_path, total_only)
    ...
    y_label = "Mean Population at Final Timestep" if ylabel is None else ylabel
    columns = [TOTAL_POPULATION_COLUMN] if total_only is True else POPULATION_COLUMNS
    legend_labels = {
