...
TTL_COLUMNS = [
    ("experimental_TTL_mean", "experimental_TTL_stdDev", "experimental", "magenta"),
    ("control_TTL_mean", "control_TTL_stdDev", "control", "cyan"),
]
TOTAL_TTL_COLUMN = ("total_TTL_mean", "total_TTL_stdDev", "total", "cyan")
...
def print_help():
    print(
        ...
        "\t-y,--ylabel\tOptional custom y-axis label. Defaults to Mean Time to Live.\n"
        ...
        "\t-t,--total-only\tPlot only total_TTL_averaged as a single line.\n"
...
def load_TTL_csv(csv_path, total_only=False):
    with open(csv_path, newline="") as input_file:
        ...
        configurations = []
        columns = [TOTAL_TTL_COLUMN] if total_only is True else TTL_COLUMNS
...
def default_output_path(input_path):
    ...
    return os.path.join(os.path.dirname(input_path), f"{base_name}_averaged_TTL.pdf")
...
def generate_bar_chart(
    ...
    configuration_column, configurations, series = load_TTL_csv(csv_path, total_only)
    ...
    y_label = "Mean Time to Live" if ylabel is None else ylabel
    columns = [TOTAL_TTL_COLUMN] if total_only is True else TTL_COLUMNS
    legend_labels = {
    ...
    if len(means) == 0:
        raise ValueError(f"CSV file {csv_path} does not contain any averaged time to live values.")
